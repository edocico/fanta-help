...
**Cosa hanno trovato le due revisioni**, oltre a quello che era già scritto. Sette rilievi di copy e nove di fase, tutti verificati contro il codice e tutti veri — per questa repo è inconsueto, e la ragione è che il lavoro era quasi tutto meccanico e i rilievi cadevano sulle poche decisioni.

- **Una giustificazione falsa, non una svista.** Avevo alzato il gradino base di `--proj-credits` scrivendo che «l'app non lo raggiunge più, perché la finestra si apre a 1440×900»: avevo letto la taglia predefinita e non il minimo, che è `minHeight: 700`. È la trappola del breakpoint che la finestra non raggiunge mai, presa dal lato dell'altezza — e la riga del `CLAUDE.md` nominava solo `minWidth`.
- **Il cremisi era sotto il pavimento del §12 dove i rifiuti si leggono.** 4,28:1 su `--surface-panel` contro un pavimento di 4,5. È lo stesso difetto che questo task ha corretto in grande, in scala minore e dentro la correzione stessa: il §12 misurava il gesso su tre superfici e il cremisi su una.
- **Tre `text-micro` erano inerti**, in conflitto con un `${band.small}` interpolato: due `font-size` sullo stesso elemento, e a decidere è l'ordine di emissione. Undici dichiarati, dodici sullo schermo.
- **Il sostantivo accanto a una cifra che conta, ancora tre volte** — la categoria che il `CLAUDE.md` registra come sfuggita a tre task di fila, e che è sfuggita anche a questo.
- **Tre numeri erano sbagliati**: l'aritmetica del docblock di `Glyph` (12+7+2+4 faceva 25 solo perché il resto era vuoto), «42 violazioni bloccanti» che sono 35, e una frase che descriveva il passo meccanico come se fosse tutto il task.

**Misurato nell'app, con `/misura-layout`.** A 1440×871, che è la finestra come si apre: `body` a **13px** con interlinea **18,2**, `.label` a **11px peso 500**, i titoli di vista in **Archivo 24px peso 500** con interlinea 26,4, le intestazioni di pannello a **18px**. La riga della tabella dati sta a **40,00px** esatti, quindi il numero che T23 aveva fissato e che il virtualizzatore dichiara non è invecchiato. La cella della board misura **22px** in modo normale e **24** in proiezione, dove la board passa da 1080 a 1400px di larghezza perché il pannello di assegnazione si smonta, e l'insieme sta in **701px su 871 senza scorrere**. Il pannello di dettaglio entra con `panel-in` in **0,15s** sulla curva del §7. Le due scorciatoie nuove rendono in `rgb(143,160,150)`, cioè `--chalk-400`, che è il colore che il §10 nomina.

A 1100×672, la finestra più piccola che l'app permette: nella vista Giocatori il titolo occupa **100,1px** e la ricerca **618,6**, senza che nessuno dei due si schiacci — il rischio che questo task correva sulla riga più densa non c'è.

**Quello che non si è potuto misurare, e perché.** La scaletta `--proj-*` veste `RosterGrid`, la lista di ripiego, che `RosterPane` monta solo quando la colonna della board scenderebbe sotto i 98px — una soglia **per squadra**, non il 1000 piatto che il §10 scrive. La lega del database di sviluppo ha **3 squadre**, quindi la sua colonna misura 236px a finestra minima e la board non cede mai: la lista proiettata non è mai comparsa. Serve una lega da dieci squadre o più, e la lista *in proiezione* comincia a esistere da quattordici — perché senza il pannello di assegnazione la board guadagna 320px. È il caso che non esiste nei dati, applicato a un layout.

**Restava da misurare, e in parte resta.** Il `CLAUDE.md` vieta di calcolare a mente le taglie di un layout, e `/misura-layout` si invoca a mano. Le tre cose che questo task ha spostato e che vanno misurate in esecuzione: l'altezza delle due barre compatte di revisione e resoconto, ora che l'`<h1>` è a 24px invece che a 13; la riga della vista Giocatori, dove il titolo divide la larghezza col campo di ricerca; e — la sola delle tre che regge un vincolo dichiarato — la scaletta `--proj-*`, dove T25 ha mosso tutti e due i lati: `--proj-credits` da 18 a 20 in proiezione, e la riga normale da 14 a 13. Il vincolo è «in proiezione entrano almeno tante squadre quante nella schermata normale», il margine misurato in T15 era 9 contro 8, e il gradino base **si raggiunge**.
