...
export default function Controls({
  ...
  return (
    <aside className="flex w-56 shrink-0 flex-col overflow-y-auto border-l border-line bg-surface-panel">
      <h2 className="label sticky top-0 border-b border-line bg-surface-panel px-3 py-2 text-micro text-fg-muted">
        Controlli
      </h2>

      {groups.length === 0 ? (
        <p className="px-3 py-3 text-base text-fg-muted">Nessuna anomalia: le rose tornano tutte.</p>
      ) : (
        <ul>
              ...
              <li key={team.id} className="border-b border-line/60">
                ...
                >
                  ...
                  {/* Quante ne ha quella squadra, non dei crediti: niente
                      ...
                      denaro. Il `text-fg-muted` è quello di prima, e la
                      taglia la dà la riga. */}
                  <Figure value={anomalies.length} className="text-fg-muted" />
                </button>
                ...
                {open && (
                  <ul className="pb-2">
                    {anomalies.map((anomaly, i) => (
                      <li key={i}>
                        <button
                          className="block w-full px-3 py-0.5 pl-8 text-left text-base text-fg-muted hover:text-fg"
